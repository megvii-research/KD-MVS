...
#filter by gimupa
parser.add_argument('--fusibile_exe_path', type=str, default='/data/Trans_cas/fusibile/fusibile')
...
def worker(rank, testlist):
    torch.cuda.set_device(rank)
    testlist_t = testlist[rank]
    for t in testlist_t:
        save_scene_depth([t])


def save_depth_mp(testlist):
    num_GPU = 4
    testlist_per_GUP = [testlist[i::num_GPU] for i in range(num_GPU)]
    args = (testlist_per_GUP,)
    torch.multiprocessing.spawn(worker, args=args, nprocs=num_GPU, join=True, daemon=False, start_method='spawn')
...
def save_scene_depth(testlist):
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            outputs = model(imgs=sample_cuda["imgs"], proj_matrices=sample_cuda["proj_matrices"],
            ...
            for filename, cam, img, depth_est, depth_stage1,depth_stage2, photometric_confidence, conf_1, conf_2 in zip(filenames, cams, imgs, \
                                                            outputs["depth"], outputs['stage1']['depth'],outputs['stage2']['depth'], outputs["photometric_confidence"],  outputs['stage1']["photometric_confidence"], outputs['stage2']["photometric_confidence"]):
                img = img[0]  #ref view
                cam = cam[0]  #ref cam
                H,W = photometric_confidence.shape
                conf_1 = cv2.resize(conf_1, (W,H))
                conf_2 = cv2.resize(conf_2, (W,H))
                depth_stage1 = cv2.resize(depth_stage1, (W,H))
                depth_stage2 = cv2.resize(depth_stage2, (W, H))
                ...
                confidence_color = visualize_depth(photometric_confidence)
...
if __name__ == '__main__':
    ...
    pcd_dir = os.path.join(args.outdir, "gipuma_fusion")
    if not os.path.isdir(pcd_dir) :
        os.makedirs(pcd_dir)
    gipuma_filter(testlist, args.outdir, pcd_dir, args.prob_threshold, args.disp_threshold, args.num_consistent,
