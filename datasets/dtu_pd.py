...
# by Yikang Ding (only for training)
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06, **kwargs):
        super(MVSDataset, self).__init__()
        self.datapath = datapath
        self.proc_depthpath = '/data/DTU/Processed_GT_depth'
    ...
    def read_mask_hr(self, filename):
        ...
        mask = np.array(read_pfm(filename)[0], dtype=np.float32)
        mask_lr = np.array(mask>1.0, dtype=np.float32)
        h, w = mask_lr.shape
        np_img_ms = {
            "stage1": cv2.resize(mask_lr, (w//4, h//4), interpolation=cv2.INTER_NEAREST),
            "stage2": cv2.resize(mask_lr, (w//2, h//2), interpolation=cv2.INTER_NEAREST),
            "stage3": mask_lr,
        }
        return np_img_ms
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            mask_filename_hr = os.path.join(self.proc_depthpath, 'depth/{}/{:0>8}.pfm'.format(scan, vid))
            depth_filename_hr = os.path.join(self.proc_depthpath, 'depth/{}/{:0>8}.pfm'.format(scan, vid))
