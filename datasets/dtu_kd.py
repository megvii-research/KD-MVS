...
class MVSDataset(Dataset):
    ...
    def read_pseudo_depth_hr(self, filename):
        # read pfm depth file
        depth_hr = np.array(read_pfm(filename)[0], dtype=np.float32)
        h, w = depth_hr.shape
        depth_lr_ms = {
    ...
    def read_pseudo_depth_mask_hr(self, filename):
        pseudo_depth = np.array(read_pfm(filename)[0], dtype=np.float32)
        mask = np.logical_and(pseudo_depth>425., pseudo_depth<905.)
    ...
    def read_sigma_hr(self, filename):
        sigma_hr = np.array(read_pfm(filename)[0], dtype=np.float32)
        h, w = sigma_hr.shape
        sigma_lr_ms = {
            "stage1": cv2.resize(sigma_hr, (w//4, h//4), interpolation=cv2.INTER_NEAREST),
            "stage2": cv2.resize(sigma_hr, (w//2, h//2), interpolation=cv2.INTER_NEAREST),
            "stage3": sigma_hr,
        }
        return sigma_lr_ms
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if not os.path.isdir(os.path.join(self.pseudo_depth, '{}'.format(scan))):
                raise FileNotFoundError
                print(f"No pseudo depth, use GT depth for {self.mode}.")
                depth_filename_hr = os.path.join(self.datapath, 'Depths_raw/{}/depth_map_{:0>4}.pfm'.format(scan, vid))
            else:
                depth_filename_hr = os.path.join(self.pseudo_depth, '{}/checked_depth/{:0>8}.pfm'.format(scan, vid))
                sigma_filename_hr = os.path.join(self.pseudo_depth, '{}/fused_sigma/{:0>8}.pfm'.format(scan, vid))
            ...
            if i == 0:  # reference view
                mask_read_ms = self.read_pseudo_depth_mask_hr(depth_filename_hr)
                depth_ms = self.read_pseudo_depth_hr(depth_filename_hr)
                sigma_ms = self.read_sigma_hr(sigma_filename_hr)
